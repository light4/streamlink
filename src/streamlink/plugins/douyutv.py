...
ROUTES = {"ws-h5": "main", "tct-h5": "backup5", "ali-h5": "backup6", "other": "other"}
...
_room_schema = validate.Schema(
    {
        "data": validate.any(
            None,
            {
                ...
                "cdnsWithName": validate.all(
                    validate.any(
                        [],
                        validate.all(
                            [
                                {"name": validate.text, "cdn": validate.text},
                            ]
                        ),
                    ),
                    validate.transform(
                        lambda cdns: dict([(e["name"], e["cdn"]) for e in cdns])
                    ),
                ),
                "multirates": validate.all(
                    validate.any(
                        [],
                        validate.all(
                            [
                                {"name": validate.text, "rate": int},
                            ]
                        ),
                    ),
                    validate.transform(
                        lambda cdns: dict([(e["name"], e["rate"]) for e in cdns])
                    ),
                ),
                "rate": int,
                "rtmp_cdn": validate.text,
            },
        ),
        "error": validate.all(
            validate.transform(int), validate.transform(lambda x: x == 0)
        ),
    },
    validate.get("data"),
)
...
_video_schema = validate.Schema(
    {
        "data": validate.any(
            ...
            validate.all(
                ...
                validate.transform(
                    lambda x: {k: v for k, v in x.get("thumb_video").items()}
                ),
            ),
        ),
        "error": validate.all(
...
class Douyutv(Plugin):
    ...
    def __js_ctx_init(self, html):
        ...
        if not self.js_ctx_initd:
            param = Douyutv.simple_re_validate(
                html, "sign_param", r"function ub98484234\(([^,]*)", 1, str
            )
            const = Douyutv.simple_re_validate(
                html,
                "sign_const",
                r"var %s=\[[^]]*\]" % param[: len(param) - 1],
                0,
                str,
            )
    ...
    def __parse_video_stream(self, url):
        ...
        if not vid:
            self.logger.warn(
                "{target} is recognized by Douyutv, but  not valid.".format(target=url)
            )
            return

        pid = Douyutv.simple_re_validate(
            html.text, "point_id", r'"point_id":(\d+)', 1, int
        )
        ...
        res = self.session.http.post(
            VAPI_URL, data=sign, cookies={"dy_did": sign["did"]}
        )
        # self.logger.debug(res.json())
        video = self.session.http.json(res, schema=_video_schema)
        for source, addr in video.items():
            self.logger.debug(
                "m3u8: name:{name}, addres:{address}".format(name=source, address=addr)
            )
            yield source, HLSStream(self.session, addr)

    def __check_online(self, room_id):
        res = self.session.http.get(
            "https://www.douyu.com/roomapi/biz/getSwitch?rid={}".format(room_id)
        )
        data = self.session.http.json(res)
        playback = data.get("data").get("playback")
        return playback.get("show") > 0 or playback.get("reason") == 'ROOM_NOT_IN_WHITE'
    ...
    def __parse_live_stream(self, url):
        ...
        if not room_id:
            ...
            return
        online = self.__check_online(room_id)
        if not online:
        ...
        def fetch_stream_info(room_id, data, rate=None, cdn=None):
            ...
            if cdn is not None and type(cdn) == str:
                data["cdn"] = cdn
            res = self.session.http.post(API_URL.format(room_id=room_id), data=data)
        ...
        for cdnName, cdn in pre_room["cdnsWithName"].items():
            for rateName, rate in pre_room["multirates"].items():
                ...
                rtmp_address = "{rtmp_url}/{rtmp_live}".format(
                    rtmp_url=real_room["rtmp_url"], rtmp_live=real_room["rtmp_live"]
                )
